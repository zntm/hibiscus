...
// src/message/counting.ts
import { channel } from "../config.js";
import { CountingSettings } from "../schema/global.js";
import { TerminalMetadata } from "../class/metadata.js";
var setCounter = async (message, client, counting, lastUser, number, reaction) => {
  counting ??= {}, counting.lastUser = lastUser, counting.number = number, counting.lastUpdate = (/* @__PURE__ */ new Date()).getTime(), await client.db.global.update(message.guild.id, { counting }), await message.react(reaction);
}, resetCounter = async (message, client, counting, description) => {
  await setCounter(message, client, counting, "", 0, "\u274C");
  let embed = client.utils.embedBuilder("Counting", "<:counting:1401915312680603698>", 9782232).setDescription(description);
  await message.reply({ embeds: [embed] });
}, run = async (message, client) => {
  if (message.author.bot || message.content.length <= 0)
    return;
  let userId = message.author.id, number = +message.content, data = (await client.db.global.find(message.guild.id))[0], counting = data?.counting;
  if (!counting) {
    await setCounter(message, client, counting, userId, 1, "\u2705");
    return;
  }
  let settings = counting?.settings ?? 0;
  if (isNaN(number)) {
  ...
  let nextNumber = data.counting.number + 1;
...
}, info = new TerminalMetadata().addChannel(channel.counting);
export {
  run,
  info
};
