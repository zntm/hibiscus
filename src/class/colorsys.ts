...
export const RGB2HSL = (r: any, g: number, b: number) => {
    ...
    switch (max) {
        case r:
            return new HSL(
                (((g - b) / d + (g < b ? 6 : 0)) / 6) * MAX.HUE,
                (l > 0.5 ? d / (2 - d) : d / (max + min)) * MAX.SVL,
                l * MAX.SVL,
            );
...
export const HSL2RGB = (h: any, s: number, l: number) => {
    ...
    return new RGB(
        _hue2Rgb(p, q, h + 1 / 3) * MAX.RGB,
        _hue2Rgb(p, q, h) * MAX.RGB,
        _hue2Rgb(p, q, h - 1 / 3) * MAX.RGB,
    );
};
...
export const HSV2RGB = (h: any, s: number, v: number) => {
    ...
    // @ts-ignore
    return new RGB(
        [v, q, p, p, t, v][mod] * MAX.RGB,
        [t, v, v, q, p, p][mod] * MAX.RGB,
        [p, p, t, v, v, q][mod] * MAX.RGB,
    );
};
...
export const RGB2CMYK = (r: any, g: number, b: number) => {
    ...
    // @ts-ignore
    return new CMYK(
        ((l - rPrime) / l).toFixed(3),
        ((l - gPrime) / l).toFixed(3),
        ((l - bPrime) / l).toFixed(3),
        k.toFixed(3),
    );
};
...
export const DEC2HEX = (decimalColor: string | number) => {
    return (
        typeof decimalColor === "string" ? +decimalColor : decimalColor
    ).toString(16);
};
