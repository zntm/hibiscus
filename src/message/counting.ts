...
const resetCounter = async (
    ...
    const embed = client.utils
        .embedBuilder("Counting", "<:counting:1401915312680603698>", 0x9543d8)
...
export const run = async (message: any, client: IClient) => {
    ...
    if (isNaN(number)) {
        if ((settings & CountingSettings.IsNumberStrict) !== 0) {
            await resetCounter(
                ...
                client,
                counting,
                "You are not allowed to send a messsage that's not a number! Number has been reset back to 1.",
            );
        }
    ...
    if (number !== nextNumber) {
        await resetCounter(
            ...
            client,
            counting,
            `That number is incorrect! The next number was **${nextNumber}**! Number has been reset back to 1.`,
        );
    ...
    if (
        (settings & CountingSettings.IsUserStrict) !== 0 &&
        counting.lastUser === userId
    ) {
        await resetCounter(
            message,
            client,
            counting,
            "You are not allowed to count multiple times in a row! Number has been reset back to 1.",
        );
