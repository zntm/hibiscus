...
export default (interaction: any) => {
    if (interaction.channel?.type === ChannelType.DM) {
        return interaction.client.utils.interactionWarning(
            interaction,
            "Commands can't be used in DMs!",
        );
    }
    ...
    if (interaction.isAutocomplete()) {
        interaction.client.commands
            ...
            .autocomplete(interaction, interaction.client);
    ...
    if (interaction.isModalSubmit()) {
        runCommand(
            ...
            "There was an error running this modal submission!",
        );
