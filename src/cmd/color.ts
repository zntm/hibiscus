...
const colors = (
    await Bun.file(join(__dirname, "../resources/color.dic")).text()
)
    .replaceAll("\r", "")
    .split("\n")
    .filter((i: string) => i !== "")
...
export const run = async (
    ...
    gradient(
        ...
        CanvasSize.Width,
        CanvasSize.Height,
        RGB2HEX(average, average, average),
        hex,
        CanvasSize.Height,
        CanvasSize.Width,
        CanvasSize.Height,
    ...
    gradient(
        0,
        0,
        CanvasSize.WidthHalf,
        CanvasSize.Height,
        "#000000",
        hex,
        CanvasSize.HeightDouble,
        CanvasSize.WidthHalf,
        CanvasSize.Height,
    );
    gradient(
    ...
    gradient(
        ...
        CanvasSize.Height,
        hex,
        RGB2HEX(Math.abs(r - 255), Math.abs(g - 255), Math.abs(b - 255)),
        CanvasSize.Height * 3,
        CanvasSize.Width,
        CanvasSize.Height,
    );
    ...
    const embed = client.utils
        ...
        .setFooter({
            text: "The image above shows hue, saturation, brightness and inverted version in order.",
        });

    interaction.editReply({
        embeds: [embed],
        files: [
            new AttachmentBuilder(await canvasThumbnail.encode("png")).setName(
                "thumbnail.png",
            ),
            new AttachmentBuilder(await canvasColors.encode("png")).setName(
                "colors.png",
            ),
        ],
    });
