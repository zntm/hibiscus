...
export const run = async (
    ...
    const seed = promptSeed
        ...
        .split("")
        .reduce(
            (d: number, v: string) => d ^ (d + v.charCodeAt(0) * 427_819),
            0,
        );
    ...
    if (rate > 0) {
        ...
        starDescription += "<:star_empty:1276821956406280304>".repeat(
            5 - Math.ceil(rate),
        );
    } else {
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(
            new SlashCommandStringOption()
                .setName("prompt")
                .setDescription(
                    "Set the prompt you want to check the star rating of",
                )
                .setRequired(true),
        ),
);
