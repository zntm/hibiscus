...
export const run = async (
    ...
    if (!subcommand) {
        return client.utils.interactionWarning(
            interaction,
            "This terminal command doesn't exist!",
        );
    }
    ...
    if (!subcommand.metadata.hasChannel(interaction.channel?.id)) {
        return client.utils.interactionWarning(
            interaction,
            "You are not in the correct channel to use this command!",
        );
    }
...
export const metadata = new CommandMetadata(
    ...
    new SlashCommandBuilder()
        .setName("terminal")
        .setDescription(
            "Run a hidden command (usually used for hidden commands or developer tools)",
        )
        .addStringOption(
        ...
        .addAttachmentOption(
            new SlashCommandAttachmentOption()
                .setName("attachment")
                .setDescription(
                    "Send an attachment to the terminal command (optional)",
                ),
        ),
);
